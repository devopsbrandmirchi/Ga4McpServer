...
describe("mapGoogleError", () => {
  ...
  it("maps inaccessible properties", () => {
    ...
    expect(error.message).toMatch(/could not be accessed/i);
  });
  ...
  it("maps invalid metrics and dimensions", () => {
    ...
    const dimensionError = mapGoogleError({
      code: 400,
      message: "Field dimension 'notARealDimension' is invalid",
    });
    expect(dimensionError.code).toBe("invalid_field");
    expect(dimensionError.message).toContain("notARealDimension");
  });
