...
export const REALTIME_DESCRIPTION = `What it does: queries GA4 realtime data for approximately the last 30 minutes.

When to use: "How many users are on the site right now?", current locations, pages being viewed now.

Required parameters: propertyId, metrics.
Optional parameters: dimensions, dimensionFilter, metricFilter, limit (default 1000, max 10000).

Returns: a clean table of current rows. There are no dateRanges.

Limitations: smaller field set than standard reports. Common realtime names: activeUsers, eventCount, keyEvents, country, city, unifiedScreenName, deviceCategory. For historical questions use ga4_run_report.`;
